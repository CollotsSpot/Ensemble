...
class _SettingsScreenState extends State<SettingsScreen> {
  ...
  Widget _buildPurgePlayersButton(
    ...
    return FutureBuilder<int>(
      future: provider.getUnavailablePlayersCount(),
        ...
        return Container(
          ...
          child: Column(
            ...
            children: [
              ...
              const SizedBox(height: 8),
              Text(
                'Remove unavailable players from previous app installs that are cluttering your player list.',
                style: textTheme.bodySmall?.copyWith(
                  color: colorScheme.onSurfaceVariant,
                ),
              ),
              ...
              SizedBox(
                ...
                child: OutlinedButton.icon(
                  onPressed: count > 0
                      ? () => _showPurgeConfirmation(context, provider, count)
                      : null,
                  icon: const Icon(Icons.delete_sweep_rounded),
                  label: Text(count > 0
                      ? 'Remove $count Unavailable Player${count > 1 ? 's' : ''}'
                      : 'No Unavailable Players'),
                  style: OutlinedButton.styleFrom(
                    foregroundColor: count > 0 ? colorScheme.error : colorScheme.onSurfaceVariant,
                    side: BorderSide(
                      color: count > 0
  ...
  Future<void> _showPurgeConfirmation(
    BuildContext context,
    MusicAssistantProvider provider,
    int count,
  ) async {
    ...
    final confirmed = await showDialog<bool>(
      ...
      builder: (context) => AlertDialog(
        title: const Text('Remove Unavailable Players?'),
        content: Text(
          'This will remove $count unavailable player${count > 1 ? 's' : ''} from Music Assistant. '
          'These are ghost players from previous app installs that are no longer active.\n\n'
    ...
    if (confirmed == true && mounted) {
      ...
      try {
        ...
        if (mounted) {
          ...
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
              content: Text(
                removed > 0
                    ? 'Removed $removed player${removed > 1 ? 's' : ''}${failed > 0 ? ' ($failed failed)' : ''}'
                    : 'No players were removed',
              ),
              backgroundColor: removed > 0 ? Colors.green : Colors.orange,
            ),
          );
          // Refresh the UI
          setState(() {});
        ...
        if (mounted) {
          ...
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
              content: Text('Failed to remove players: $e'),
              backgroundColor: Colors.red,
            ),
          );
        }
